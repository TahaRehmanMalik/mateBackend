...
const userCollection=collection(db,"mobilePhones");


const getMobilePhone=async()=>{
    try {
        const response=await getDocs(userCollection);
        console.log("mobilePhone  data is fetch",response);
        return response;
    } catch (error) {
        console.log("The error in mobilePhone is",error);
        throw error;
    }
}
module.exports={getMobilePhone};